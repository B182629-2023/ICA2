...
print("\n\nWelcome to the Protein Family Entrez Search and Sequence Retrieval Program.")
print("Here, you will be able to query a taxonomic group to generate the list of protein families present for your chosen tax group.")
print("You will also be able to search for a specific protein family of choice and view its prevalence in your taxonomy group.")
print("If you choose, you can use one of these identified protein families in your subsequent Enterz search to obtain relevant protein sequences.\n\n")



# Asks the user if they would like to start the program, if not, exit
while True:
...
# Create analysis directory and change to analysis directory
os.chdir(os.environ['HOME'])
homespace = os.environ['HOME']
os.mkdir("conservation_analysis")
os.chdir("conservation_analysis")

print("\nConservation analysis directory created in ", homespace, ". Changing directory...\n")


# Initially sets flag to True, when flag set to false in loop it breaks out of the outer loop.
...
while break_out_flag1:
	...
	# Searches for entries with taxnomic group with the swissprot filter and collects accession IDs
		...
		with open('tax_accessions.txt', 'r') as tax_accessions:
			...
			if len(tax_accs) == 1:
				print("There are 0 protein results for " + tax_group + ". Please enter another taxonomic group.\n")
				break
			else:
				print("\nThere are " + str(len(tax_accs)-1) + " protein entries for " + tax_group)
				break
	while True:
...
print(str(len(family_df)) + " out of " + str(len(tax_accs)-1) + " protein entries contain protein family information.")
...
# Creates empty lists to append values in the for loop
accessionid = []
proteinfam = []
nomatch_accession = []
nofam_accession = []
...
break_out_flag4 = True

while break_out_flag4:
	...
	# Some proteins contain a family name in the protein name even though it is not a part of the family e.g. kinase INHIBITOR
	# Iterates through all accessions to ensure that it truly belongs in the protein family.
	with open('prot_accessions.txt', 'r') as input_file2, open('new_prot_accessions.txt', 'w') as output_file2:
		...
		if len(prot_accessions) <= 1:
			# Len(prot_accessions) is always +1 compared to true number of accessions in file
			print("No proteins with name " + protein_family + " were found.")
			break

		elif len(prot_accessions) > 2:
			print(str(len(prot_accessions)-1) + " protein entries containing name " + protein_family + " identified.\n\nFetching protein family data for each entry...\n")
			for accession in prot_accessions:
				# Finds protein family information for each entry and cleanes lines
				...
				# Converts result into readable text
				esearch_result = acc_esearch.stdout.strip()

				if not esearch_result:
					print("Accession ID: " + accession + " does not contain protein family information. Skipping accession...\n")
					nofam_accession.append(accession)
				clean_result = esearch_result.replace("\n", " ").replace(". ","\n").replace("[SIMILARITY] Belongs to the ","\n").replace(". ","\n").replace("[SIMILARITY] In the ", "\n").replace("--","")

				# Find family string in rows, append family name and accession ID to lists
				for row in clean_result.split("\n"):
					if string in row:
						if protein_family.lower() in row.lower():
							...
							nomatch_accession.append(accession)

				# Adds up all appended accessions regardless of family status, when this value is less than total accessions, loop continues
				if (len(accessionid)+len(nomatch_accession)+len(nofam_accession)) < (len(prot_accessions)-1):
					print(str(len(accessionid)+len(nomatch_accession)+len(nofam_accession)) + "/" + str(len(prot_accessions)-1) + " accessions iterated.\n")
					pass

				# if else, force exit loop
				else:
					print(str(len(accessionid)+len(nomatch_accession)+len(nofam_accession)) + "/" + str(len(prot_accessions)-1) + " accessions iterated.\n")
					print("Protein family abalysis complete!\n")
					print(str(len(accessionid)) + "/" + str(len(prot_accessions)) + " protein entries belong to the " +  protein_family + " family.")
					break
	...
	prot_fam_data = {'Accession ID': accessionid, 'Protein Family': proteinfam}
	new_prot_fam_df = pd.DataFrame(prot_fam_data)
	int_protein_family_df = new_prot_fam_df.loc[new_prot_fam_df['Protein Family'].str.contains(protein_family, case=False)]
	int_protein_family_df_counts = int_protein_family_df.groupby('Protein Family')['Accession ID'].nunique().reset_index()
	int_protein_family_df_counts_sort = int_protein_family_df_counts.sort_values('Accession ID', ascending=False)


	if len(int_protein_family_df_counts) > 1:
		print("\n\nHere are the unique protein families which contain the name " + protein_family + " and their protein counts.\n")
		print(int_protein_family_df_counts_sort)
		...
		if continue_protein_family == "yes":
			...
			with open("sequences.fa", "w") as output_file:
				...
				for accession in accessionid:
					...
					print("Protein sequence generated for accession ID " + accession)

			print("\nProtein sequences generated! - see 'sequences.fa'.")
			break_out_flag4 = False
			break
